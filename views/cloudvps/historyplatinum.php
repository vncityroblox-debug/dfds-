...
$where = ' `id` > 0 AND `user_id` ="' . $data_user['id'] . '" AND `site` = "CLOUDNEST"';
...
if (!empty($_GET['select_status_vps'])) {
    ...
    $listCloudVpsUser = $db->get_list("SELECT * FROM `tbl_purchased_cloudvps` WHERE `user_id` = '" . $data_user['id'] . "' AND `site` = 'CLOUDNEST' ORDER BY `id` DESC");
    $liststatus = array();
        ...
        <div class="row gy-4 dashboard-row-wrapper">
            <div class="col-md-12">
                <form method="GET" action="" class="row">
                    ...
                    <div class="col-lg col-md-4 col-6">
                        <a href="/user/history/platinum" class="shop-widget-btn mb-2"><i class="far fa-trash-alt"></i><span>Bỏ lọc</span></a>
                    </div>
                </form>
                <div class="overflow-x-auto">
                            ...
                            <?php foreach ($listCloud as $row) :
                                ...
                                ?>
                                    <tr onchange="updateForm('<?= $row['id']; ?>')">
                                        ...
                                        <td class="py-2 px-4">
                                            <button onclick="location.href='/user/history/vps/platinum/<?= $row['id'] ?>';" class="btn btn-dark btn-sm"><i class="bx bx-cog mr-1"></i>Quản lý</button>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                    ...
                    if ($total > $sotin1trang) {
                        echo  pagination_client("/user/history/platinum?ip=" . $ip . "&purchase_date=" . $createdate . "&", $from, $total, $sotin1trang);
                    } ?>
                    </div>
                </div>
            </div>
