...
if (isset($_GET['id'])) {
    $id = Anti_xss($_GET['id']);
    $check = $db->get_row("SELECT * FROM `tbl_purchased_cloudvps` WHERE `id` = '{$id}' AND `user_id` = '{$data_user['id']}' AND `site` = 'CLOUDNEST' ");
    if (!$check) {
        new Redirect('/user/history/vps/platinum');
    }
    ...
    $row = $db->get_row("SELECT * FROM `tbl_purchased_cloudvps` WHERE `id` = '{$id}' AND `user_id` = '{$data_user['id']}' AND `site` = 'CLOUDNEST'");
    if (!$row) {
        new Redirect('/user/history/vps/platinum');
    }
