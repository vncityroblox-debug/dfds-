...
if ($action) $conditions[] = "action LIKE '%$action%'";
if ($ip) $conditions[] = "ip LIKE '%$ip%'";
if ($purchase_date) {
...
$total_items = $db->get_row("SELECT COUNT(*) as total FROM logs WHERE $where")['total'] ?? 0;
$logs = $db->get_list("SELECT * FROM logs WHERE $where LIMIT $limit OFFSET $offset");

$url = "/user/log?action=$action&ip=$ip&purchase_date=$purchase_date&";
?>
        ...
        <div class="row">
            <div class="col-md-12">
                <form method="GET" action="" class="row">
                    ...
                    <div class="col-lg col-md-4 col-6">
                        <input class="form-control shadow-none col-sm-2 mb-2" name="ip" type="text" value="<?= htmlspecialchars($ip) ?>" placeholder="IP">
                    </div>
                    ...
                    <div class="col-lg col-md-4 col-6">
                        <a href="/user/log" class="shop-widget-btn mb-2"><i class="far fa-trash-alt"></i><span>Bỏ lọc</span></a>
                    </div>
                </form>
                <div class="overflow-x-auto">
                        ...
                        <table class="w-100 dashboard-table table text-dark">
                            <thead class="pb-3">
                                <tr>
                                    ...
                                    <th scope="col" class="py-2 px-4">Thiết bị</th>
                    ...
                    <?php foreach ($logs as $log): ?>
                                    <tr>
                                        ...
                                        <td class="text-dark"><?= htmlspecialchars($log['action']) ?></td>
                                        <td class="text-dark"><?= htmlspecialchars($log['ip']) ?></td>
                                        <td class="text-dark"><?= htmlspecialchars($log['device']) ?></td>
                                        <td><span class="status-badge in-active"><?= htmlspecialchars($log['create_date']) ?></span></td>
                                    </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
                    <tr>
