...
if (isset($_GET['id']) && $data_user['level'] == 'admin') {
    $id = Anti_xss($_GET['id']);
    $row = $db->get_row("SELECT * FROM `tbl_addon_vps` WHERE `id` = '" . $id . "'");
    if (!$row) {
        new Redirect("/cpanel/vps/addon");
    }
...
if (isset($_POST['updatePlan']) && $data_user['level'] == 'admin') {
    ...
    $prices['pricing']['monthly']['amount'] = Anti_xss($_POST['monthly']);
    $prices['pricing']['twomonthly']['amount'] = Anti_xss($_POST['twomonthly']);
    $prices['pricing']['quarterly']['amount'] = Anti_xss($_POST['quarterly']);
    $prices['pricing']['semi_annually']['amount'] = Anti_xss($_POST['semi_annually']);
    $prices['pricing']['annually']['amount'] = Anti_xss($_POST['annually']);
    $prices['pricing']['biennially']['amount'] = Anti_xss($_POST['biennially']);
    $prices['pricing']['triennially']['amount'] = Anti_xss($_POST['triennially']);

    $isUpdate = $db->update("tbl_addon_vps", array(
...
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-name fw-semibold fs-18 mb-0"><i class="fa-solid fa-layer-group"></i> Chỉnh sửa Addon <?= $row['name'] ?> VPS</h1>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <div class="card custom-card">
                    <div class="card-header d-flex align-items-center">
                        <h5 class="card-title mb-0 flex-grow-1">CHỈNH SỬA ADDON <?= $row['name'] ?> VPS - #<?= $row['id'] ?></h5>
                    </div>
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="card-footer">
                            <a href="/cpanel/vps/addon" class="btn btn-danger waves-effect">QUAY LẠI</a>
