...
if (!empty($_GET['status'])) {
    ...
    $listCloudVpsUser = $db->get_list("SELECT * FROM `tbl_purchased_cloudvps` ORDER BY `id` DESC");
    $liststatus = array();
...
if (!empty($_GET['ip'])) {
    $ip = Anti_xss($_GET['ip']);
    $listCloudVpsUserIp = $db->get_list("SELECT * FROM `tbl_purchased_cloudvps` ORDER BY `id` DESC");
    $listip = array();
...
$listCloudVps = $db->get_list("SELECT * FROM `tbl_purchased_cloudvps`");
$array = array();
...
$urlDatatable = pagination(("/cpanel/vps/history&limit=" . $limit . "&shortByDate=" . $shortByDate . "&ip=" . $ip . "&status=" . $select_status_vps . "&"), $from, $totalDatatable, $limit);

$total_vps_active = $db->get_row("SELECT COUNT(id) as total FROM tbl_purchased_cloudvps WHERE `status` = 'on' ")['total'] ?? 0;
$total_vps_expire = $db->get_row("SELECT COUNT(id) as total FROM tbl_purchased_cloudvps WHERE `status` = 'expire'")['total'] ?? 0;
$total_price = $db->get_row("SELECT SUM(total_price) as total FROM tbl_purchased_cloudvps")['total'] ?? 0;
$total_cost = $db->get_row("SELECT SUM(total_cost) as total FROM tbl_purchased_cloudvps")['total'] ?? 0;
?>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6 mb-2">
                                    <button class="btn btn-info waves-effect" type="submit" name="filter">Tìm
                                        kiếm</button>
                                    <a href="/cpanel/vps/history" class="btn btn-danger waves-effect">Tất
                                        cả</a>
                                </div>
                            </div>
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="/cpanel/vps/history/edit/<?= $row['id'] ?>">
