...
$listLogs = $db->get_list("SELECT * FROM `product_withdraw` WHERE $where $order_by LIMIT $from,$sotin1trang ");
active_license()
?>
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">Product Withdraw</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="#">Product Program</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Product Withdraw</li>
                    </ol>
                </nav>
            </div>
        </div>
                            ...
                            <div class="col-sm-12 col-md-12 mb-3">
                                <div class="pagination-style-1">
                                    <div class="d-flex justify-content-center">
                                        <?php
                                        $total = $db->num_rows("SELECT * FROM `product_withdraw` WHERE $where");
                                        if ($total > $sotin1trang) {
                                            echo '<center>' . pagination("/cpanel/product/withdraw?user_id=$userid&username=$username&content=$content&createdate=$createdate&limit=$limit&shortByDate=&", $from, $total, $sotin1trang) . '</center>';
                                        } ?>
                                    </div>
                                </div>
                            </div>
...
function change() {
    $('#change').html('Đang xử lý...').prop('disabled',
        true);
    $.ajax({
        url: "/model/admin/withdraw-product",
        method: "POST",
        dataType: "JSON",
        data: {
            id: $("#iddiamond").val(),
