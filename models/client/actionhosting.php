...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($user) {
        try {
            ...
            $check_history = $db->get_row("SELECT * FROM `purchased_hosting` WHERE `id` = '$param' AND `user_id` = '" . $data_user['id'] . "'");
            if (!$check_history) {
            ...
            switch ($action) {
                case '2':
                    if($check_history['status'] == 'expired'){
                        die(JsonMsg('error', 'Gói hosting đã hết hạn, gia hạn để tiếp tục sử dụng chức năng'));
                    }
                ...
                case '3':
                    ...
                    if (isset($data['metadata']['result'])) {
                        if ($data['metadata']['result'] == 1) {
                            $username = $check_history['username'];
                            $password = $check_history['password'];
                            $domain = $check_history['domain_name'];
                            $email = $check_history['email'];
                            if ($server_whm['username'] == 'root') {
                                $package = $info_package['package_name'];
                            } else {
                                $package = $server_whm['username'] . '_' . $info_package['package_name'];
                            }
                            $datajson = createHostingViaAPI($server_whm['ip'], $server_whm['username'], $server_whm['password'], $username, $domain, $email, $package, $password);
                            if (isset($datajson['metadata']['result'])) {
                                if ($datajson['metadata']['result'] == 1) {
                                    $update = $db->update("purchased_hosting", array(
                                        'status' => 'active',
                                    ), " `id` = '$param' ");
                                    if ($update) {
                                        die(JsonMsg('success', 'Đã cài lại hosting của bạn thành công'));
                                    } else {
                                        die(JsonMsg('error', 'Không reset. Vui lòng liên hệ ADMIN'));
                                    }
                                }
                            }
                        } else {
                ...
                case '5':
                    ...
                    if ($money > $data_user['money']) {
                        die(JsonMsg('error', 'Số dư của bạn không đủ ' . format_cash($money) . ', vui lòng nạp thêm để thực hiện'));
                    }
                    if ($server_whm['username'] == 'root') {
                ...
                case '7':
                    ...
                    if (isset($data['metadata']['result'])) {
                        if ($data['metadata']['result'] == 1) {
                            $db->insert("logs", ["user_id" => $data_user["id"], "ip" => myip(), "device" => $Mobile_Detect->data_userAgent(), "createdate" => gettime(), "action" => 'Thực hiện xóa hosting có tên miền ' . $check_history['domain_name']]);
                            $isRemove = $db->remove("purchased_hosting", " `id` = '$param' ");
                            die(JsonMsg('success', "Hệ thống đã xóa hosting thành công. Cảm ơn bạn đã sử dụng dịch vụ"));
                        } else {
                            die(JsonMsg('error', $data['metadata']['reason']));
                        }
                    } else {
                        die(JsonMsg('error', 'Không thể xóa hosting. Vui lòng liên hệ ADMIN'));
                    }
                    break;
                case '8':
                    if($check_history['status'] == 'expired'){
                    ...
                    if (!isset($_POST['domain']) || empty($_POST['domain'])) {
                        die(JsonMsg('error', "Nhập tên miền cần thêm"));
                    }
                    ...
                    // }
                    $data = addAddonDomainViaAPI($server_whm['ip'], $server_whm['username'], $server_whm['password'], $check_history['username'], $domain);
