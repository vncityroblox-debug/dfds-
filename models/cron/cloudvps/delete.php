...
$array = array();
$listCloudVps = $db->get_list("SELECT * FROM `tbl_purchased_cloudvps` WHERE `status` = 'expire' AND `notification_delete` = 'no'");
...
if (!empty($array)) {
    ...
    if (isset($result['error']) && $result['error'] == 0) {
        foreach ($result['data'] as $infovps) {
            ...
            if ($infovps['vps-status'] == 'delete_vps') {
                $htmlTemplate = file_get_contents(DOMAIN . '/libs/mails/cloudvps/delete', false, stream_context_create($arrContextOptions));
                ...
                $chu_de = 'Thông báo: Dịch vụ của bạn đã bị xóa';

                $result = sendCSM($mail_nhan, $ten_nhan, $chu_de, $htmlContent);

                $db->query("UPDATE tbl_purchased_cloudvps SET notification_delete = 'yes' WHERE vps_id = '{$infovps['vps-id']}'");
            }
